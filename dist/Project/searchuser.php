<?php
include('Shopify_RegisterPage/session.php');
$connection = mysql_connect("localhost", "root", "");
// Seleksi Database
$database="database market";
$db = mysql_select_db($database);
$search=$_POST['cari'];
$cariquery=mysql_query("select * from produk where namaproduk LIKE '$search%'",$connection);
$searchrow=array();
$a=0;
$bykrow = mysql_num_rows($cariquery);
while ($carirow = mysql_fetch_assoc($cariquery)) {
    $searchrow [$a]=$carirow['produkID'];
    $a++;
}
?>
<!DOCTYPE html>
...
<body>
        ...
        <nav>
            <div class="w3-top">
                <ul class="w3-navbar w3-left-align w3-large">
                    ...
                    <li><a href="Shopify-HomePage.php" class="homecolor"><i class="fa fa-search w3-margin-right"></i>Search</a></li>
    ...
    <div class="w3-content w3-container w3-padding-64">
        <div class="row">
            ...
            <h4>Hasil Search: <?php echo $search;?></h4>
            
            <?php
        
            for($j=0;$j<$bykrow;$j++)
            {
                $cariquery = mysql_query("SELECT * FROM produk WHERE produkID='$searchrow[$j]'",$connection);
                $carirows = mysql_fetch_assoc($cariquery);
                $kat=$carirows['kategori'];
                $kat=str_replace(' ','',$kat);
            ?>
            <div class="col-sm-2 col-md-3">
                <div class="thumbnail">
                    <?php echo "<img src=$kat/".$carirows['Foto'].">";?>
                    <div class="caption">
                        <h6><?php echo $carirows['namaproduk'];?></h6>
                        <p><?php echo "Rp.".number_format($carirows['harga'],0,",",".");?></p>
                        <form action="Shopify-SecondPage/OrderUser.php" method="post">
                            <input type="hidden" name="id" value="<?php echo $carirows['produkID'];?>">
